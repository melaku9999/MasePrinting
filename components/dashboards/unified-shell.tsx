...
export function UnifiedShell({
  ...
  const currentPageLabel = allItems.find((item) => item.id === activeTab)?.label || "Dashboard"
      ...
      <div className="flex items-center gap-3 px-4 h-16 border-b border-border/60 shrink-0">
        ...
        {(!sidebarCollapsed || isMobile) && (
          <div className="overflow-hidden">
            <h2 className="text-sm font-semibold text-foreground truncate leading-tight">{appName}</h2>
            <p className="text-[11px] text-muted-foreground truncate leading-tight">{appSubtitle}</p>
          </div>
        )}
        {!isMobile && (
          <button
            ...
            <div className="flex items-center justify-between h-full px-4 lg:px-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground hidden sm:block">{user.role.charAt(0).toUpperCase() + user.role.slice(1)} Portal</p>
                </div>
              </div>
