...
import { Search, Building, DollarSign, FileText, Calendar, CheckCircle2, AlertCircle, Users, Clock } from "lucide-react"
import { servicesApi, serviceAssignmentsApi } from "@/lib/api"
import type { Service } from "@/lib/auth"
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog"
import { ServiceAssignmentForm } from "../services/service-assignment"
...
export function CustomerServices({ customerId, onServiceSubscribed }: CustomerServicesProps) {
    ...
    try {
      ...
      if (servicesData) {
        ...
        const transformedServices: Service[] = servicesData.map((service: any) => ({
          ...
          subtasks: (service.subtask_templates || []).map((subtask: any) => ({
  ...
  const openAssignmentDialog = (service: Service) => {
  ...
  const handleSaveAssignment = async (assignment: any) => {
    try {
      setLoading(true)
      const response = await serviceAssignmentsApi.create(assignment)
      if (response.success) {
  ...
  const removeFromPending = async (serviceId: string) => {
    try {
      ...
      if (response) {
        toast.success("Removed from pending approval")
      }
      await fetchServices()
    } catch (error: any) {
      console.error("Error removing from pending list:", error)
      toast.error("Error removing from pending list")
    } finally {
  ...
  const isCustomerSubscribed = (service: Service) => {
    if (service.subscribed_customers) {
      return service.subscribed_customers.some((customer: any) => 
  ...
  const isCustomerPending = (service: Service) => {
    if (service.pending_customers) {
      return service.pending_customers.some((customer: any) => 
        customer.id.toString() === customerId.toString()
      )
    }
    return false
    ...
    return (
      <div className="p-8 rounded-3xl bg-rose-50 border border-rose-100 text-center space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-rose-600 text-sm font-medium text-xs">{error}</p>
        </div>
  ...
  return (
    <div className="space-y-8 p-1">
      ...
      {filteredServices.length > 0 ? (
              ...
              <CardContent className="p-6 flex-1 flex flex-col space-y-4">
                ...
                <div className="pt-4 border-t border-slate-50">
                  {isCustomerSubscribed(service) ? (
                    ...
                    <Button
                      onClick={() => openAssignmentDialog(service)}
                      disabled={subscribing[service.id]}
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-white rounded-3xl border border-slate-100 border-dashed">
          ...
          <p className="text-slate-500 text-center max-w-sm mb-8 font-medium">
            No service parameters matched your query in the enterprise registry.
          </p>
          {searchTerm && (
            <Button 
              variant="ghost" 
      ...
      <Dialog open={isAssignmentDialogOpen} onOpenChange={setIsAssignmentDialogOpen}>
        <DialogContent className="max-w-5xl h-[90vh] overflow-y-auto font-sans">
          <DialogHeader>
